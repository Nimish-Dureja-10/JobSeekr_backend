...
export const login = async (req,res) => {
    try {
        ...
        return res.status(200).cookie("token",token,{maxAge:3*24*60*60*1000,httpsOnly:true,sameSite:'strict'}).json({success:true,user,message:`Welcome back ${user.fullname}`});
    }catch(error) {
        console.log(error);
    }
};
